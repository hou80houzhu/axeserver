(function () {
    ...
    var serialize = {
        ...
        queryInfo: function (str) {
            var a = str.indexOf("?"), b = str.indexOf("#"), querystring = "", hashstring = "";
            if (a !== -1 && b !== -1) {
                if (a > b) {
        ...
        hashObject: function (str) {
            var a = str.indexOf("?"), b = str.indexOf("#"), hashstring = "";
            if (a !== -1 && b !== -1) {
                if (a > b) {
                    hashstring = str.substring(b + 1, a);
                } else {
    ...
    plugin.prototype.delegateBtn = function () {
        var ths = this;
    ...
    plugin.prototype.delegateAll = function () {
        this.delegateEvent();
        this.delegateFind();
        this.delegateBtn();
        return this;
    ...
    _system.prototype.boot = function (selector, option) {
        if (arguments.length === 1) {
            $("body").Boot(selector);
        } else if (arguments.length === 2) {
            $(selector).Boot(option);
        }
    };
    ...
    packet.getPacketInfo = function (str) {
        ...
        if (a && a.length > 0) {
            ...
            for (var o = 0; o < tp.length; o++) {
                ...
                var d = a.split(" ");
                if (d.length >= 2) {
                    var key = d[0].substring(1, d[0].length), value = d[1].substring(0, d[1].length - 1), suffix = null;
                    if (!n[key]) {
                        n[key] = [];
                    }
                    ...
                    switch (key) {
                        ...
                        case "require":
                            n._depends.push(t[0]);
                            n.children.push(t[0]);
                            value = basepath + t[0].replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                            break;
                        case "include":
                            n.children.push(t[0]);
                            value = basepath + t[0].replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                        ...
                        case "css":
                            if (value.indexOf("http") === -1) {
                                if (value.indexOf("/") === -1) {
                                    value = basepath + value.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".css");
                                } else {
                        ...
                        case "js":
                            if (value.indexOf("http") === -1) {
                                if (value.indexOf("/") === -1) {
                                    value = basepath + value.replace(packet.isdot, "/") + (suffix ? "." + suffix : ".js");
                                } else {
                                    value = basePath + value;
                                }
                            }
        ...
        brooder.ajax({
            ...
            success: function (e) {
                ...
                for (var i = 0; i < aa.template.length; i++) {
                    ...
                    if (!packet.templatemapping[path]) {
                            ...
                            brooder.ajax({
                                ...
                                success: function (txt) {
                                    ...
                                    for (var i = 0; i < b.length; i++) {
                                        var c = b[i].substring(5, b[i].length - 4);
                                        _dommapping[c] = p[i + 1].replace(packet.isNotec, "");
                                    }
                                    packet.templatemapping[path] = _dommapping;
                        ...
                        aa.template[i].value = packet.templatemapping[path];
                    }
                }
    ...
    brooder.Boot = function (option) {
        if (!isboot) {
            console.log("[brooder] version:" + brooderInfo.version + " line:" + brooderInfo.line + " gzip:" + brooderInfo.gzip + " debug:" + baseMapping.debug + " basePath:" + baseMapping.basePath);
    ...
    query.prototype.Boot = function (optionName) {
