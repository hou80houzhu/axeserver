/*!
 * @packet mart.form;
 * @require mart.util.toast;
 * @require mart.util.file;
 * @require mart.tree;
 * @css mart.style.form;
 * @js contenter.lib.ace.ace;
 * @template mart.template.formtemplate;
 */
Module({
    name: "baseform",
    extend: "viewgroup",
...
Module({
    ...
    layout: "<div class='label-block'><%=data.option.label;%></div>" +
            ...
            "<div class='tableis'>" +
            "<div class='ts' style='font-size:30px;position:absolute;left:0;right:0;top:50%;text-align:center;'>单击左侧菜单项进行编辑</div></div>" +
    ...
    btn_checkbtn: function () {
        var ths = this;
        this.getLastChild().getValues().done(
                function (data) {
                    var xid = ths.getUUID();
                    if (data) {
...
Module({
    ...
    init: function () {
        ...
        editor.$blockScrolling = 1.7976931348623157E+10308;
        editor.focus();
        this.editor = editor;
        this.setValue(this.option.value || "");
    },
