...
Module({
    ...
    event_flipBlank: function (e) {
        this.addChild({
            type: "@.blank",
            option: {url: basePath+e.data.url, title: e.data.name}
        });
    }
});
Module({
    ...
    focusItem: function (index) {
        ...
        for (var i = 0; i < this.items.length; i++) {
            if (i !== index) {
                this.items[i].removeClass("hover");
            } else {
                this.items[i].addClass("hover");
            }
        }
    },
    _resize: function () {
...
Module({
    ...
    layout: domstr("@skin", "maincontainer"),
    init: function () {
        var ths = this;
        var str = $.template(domstr("@skin", "maincontainermenu")).render(this.parameters.mapping.list);
    ...
    showMask:function(data){
        this.dom.find(".maincontainer-mask").show().children(0).html("<i class='"+data.img+"'></i>");
    },
    hideMask:function(){
        this.dom.find(".maincontainer-mask").hide();
    },
    resize: function () {
