/*!
 * @packet question.datepicker; 
 * @template question.template.question;
 */
$.formatDate = function (format, num) {
    if (!format) {
    ...
    var a = new Date(new Date().getTime() + 86400000 * num);
    var hour = a.getHours(), minite = a.getMinutes(), sound = a.getSeconds(), month = a.getMonth() + 1, day = a.getDate(), year = a.getFullYear();
...
Module({
    ...
    extend: "viewgroup",
    className: "datep",
    layout: module.getTemplate("@question", "datep"),
    option: {
        withTime: false,
        id: "",
        label: "",
    ...
    setValue: function (a) {
        ...
        this.option.value = this.value;
        this.finders("btn").html(this.value);
    }
});
Module({
    name: "datepicker",
    extend: "viewgroup",
    className: "dpicker",
    layout: module.getTemplate("@question", "dpicker"),
    option: {},
    init: function () {
        this.addChild({
            type: "@.datepanel",
...
Module({
    ...
    extend: "view",
    className: "datepanel",
    template: module.getTemplate("@question", "datepanel"),
    option: {
    ...
    find_shour: function (dom) {
        dom.button(function (e, ths) {
            ths.finders("phour").show();
        }, this);
    },
    find_stime: function (dom) {
        dom.button(function (e, ths) {
            ths.finders("psecond").data("time", "time").show();
        }, this);
    },
    find_ssecond: function (dom) {
        dom.button(function (e, ths) {
            ths.finders("psecond").data("time", "second").show();
        }, this);
    },
    find_second: function (dom) {
        dom.button(function (e, ths) {
            var a = ths.finders("psecond");
            if (a.data("time") === "time") {
                ths.Minite = parseInt($(this).html());
                ths.finders("stime").children(0).html(ths.Minite);
            } else {
                ths.Second = parseInt($(this).html());
                ths.finders("ssecond").children(0).html(ths.Second);
            }
            a.hide();
        }, this);
    },
    find_hour: function (dom) {
        dom.button(function (e, ths) {
            ...
            ths.finders("shour").children(0).html(ths.Hour);
            ths.finders("phour").hide();
        }, this);
    },
    ...
    currentyears: function () {
        ...
        this.yearbody.html($.template(module.getTemplate("@question", "datepanelyear")).render(t));
        this.delegate();
    },
    prevyears: function () {
    ...
    refresh: function () {
        ...
        if (this.option.withTime) {
            this.finders("stime").children(0).html(this.Minite);
            this.finders("shour").children(0).html(this.Hour);
            this.finders("ssecond").children(0).html(this.Second);
        }
        this.dbody.html($.template(module.getTemplate("@question", "datepanelbody")).render(a));
        this.delegate();
    },
    getDate: function () {
        return this.formate();
